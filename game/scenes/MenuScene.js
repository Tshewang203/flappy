...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(GAME_WIDTH / 2, 468, '🎧 Sound Settings', {
      fontFamily: 'Orbitron', fontSize: '11px', color: COLORS.silver,
    ...
    if (player?.name) {
      ...
      this.add.text(GAME_WIDTH / 2, 555, `${roleIcon} ${player.name} · ${player.department} · ${yearStr}`, {
        fontFamily: 'Inter', fontSize: '11px', color: 'rgba(255,255,255,0.9)',
    ...
    if (cstMsg) {
      this.add.text(GAME_WIDTH / 2, GAME_HEIGHT - 55, `💬 "${cstMsg.text}"`, {
        fontFamily: 'Inter', fontSize: '9px', color: 'rgba(255,255,255,0.7)',
